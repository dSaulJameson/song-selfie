...
export function getObjectStorageConfig() {
  return {
    gatewayUrl: getFirstEnv("R2_MEDIA_GATEWAY_URL")?.replace(/\/+$/, ""),
    gatewaySecret: getFirstEnv("R2_MEDIA_GATEWAY_SECRET"),
    publicBaseUrl: getFirstEnv("R2_MEDIA_PUBLIC_BASE_URL")?.replace(/\/+$/, ""),
  };
}
